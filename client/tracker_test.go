...
func TestTrackerClient(t *testing.T) {
	Convey("Should make an HTTP GET request and receive response", t, func() {
		...
		tReq := structure.NewTrackerRequest(metainfo)
		tResp, err := tc.MakeAnnounceRequest(tReq, "started")
		So(tc, ShouldNotBeNil)
		So(err, ShouldBeNil)
		So(tResp, ShouldNotBeNil)
		So(tResp.Complete, ShouldEqual, 1107)
	})

	Convey("Should handle failures", t, func() {
		ts := newTestServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			w.WriteHeader(200)
			w.Header().Set("Content-Type", "text/plain")
			fakeResp := "d14:failure reason8:an errore"
			fmt.Fprint(w, fakeResp)
		}))
		tc := NewTrackerClient()
		tc.HTTP.Transport = &http.Transport{
		...
		So(tResp, ShouldNotBeNil)
		So(tResp.FailureReason, ShouldEqual, "an error")
	})
}
